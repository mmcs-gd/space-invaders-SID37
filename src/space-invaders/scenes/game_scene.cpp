...
namespace SpaceInvaders {
    ...
    GameScene::GameScene(Volumatrix::Game& game):
        ...
        player = std::make_shared<InvaderEntity>(Volumatrix::Point{1, int(player_location), 200}, &animations[2]);
        player_bullet = std::make_shared<InvaderEntity>(Volumatrix::Point{4, 10, 200 - 10}, &animations[3]);
    ...
    void GameScene::Tick(float dt) {
        ...
        if (bullet_location > 0) {
            ...
            if (player_bullet->location.z > 10) {
                // game.grid.StoreLight(bullet_light, player_bullet->location - Volumatrix::Point{4, 10, 10});
                game.grid.AddLightSource({100, 100, 0}, player_bullet->location, player_bullet->location + Volumatrix::Point{1, 1, 4});
            }
        }
    }
}
