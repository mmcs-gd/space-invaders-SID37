...
Graphics::Graphics(Screen& screen, int x_size, int y_size, int z_size):
        screen(screen),
        background_shader(Shaders::vertex_shader, Shaders::background_fragment_shader),
        scene_shader(Shaders::vertex_shader, Shaders::scene_fragment_shader),
        grid_x_size(x_size),
        grid_y_size(y_size),
        grid_z_size(z_size) {
...
void Graphics::UpdateGrid(const GridPoint* grid) {
    glBindTexture(GL_TEXTURE_3D, scene_grid_texture_object);
    glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA8UI, grid_x_size, grid_y_size, grid_z_size, 0,
        GL_RGBA_INTEGER, GL_UNSIGNED_BYTE, grid);
    checkOpenGLerror();
    glFlush();
}


void Graphics::UpdateMaterials(const GridMaterial* matrials) {
    Logger::Log("Graphics", "Materials updating...");
    glBindBuffer(GL_UNIFORM_BUFFER, scene_materials_block_buffer);
    glBufferData(GL_UNIFORM_BUFFER, sizeof(GridMaterial) * 16, matrials, GL_STATIC_DRAW);
    checkOpenGLerror();
    glFlush();
}
