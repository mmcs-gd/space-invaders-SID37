...
namespace VoxelEditor {
    ...
    MainScene::MainScene(Volumatrix::Game& game):
            game(game),
    ...
    void MainScene::EventKeyDown(SDL_KeyCode key) {
    ...
    void MainScene::EventWheel(int delta) {
        if (delta == 0) return;
    ...
    void MainScene::EventButtonDown(MouseButtonType button, int x, int y) {
    ...
    void MainScene::EventMove(int x, int y) {
        if (result.Get(depth, location_x, location_y) > 1) {
            result.Get(depth, location_x, location_y) -= 2;
        }
